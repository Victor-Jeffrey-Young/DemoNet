...
appids = [(576,'2582320'),(577,'2357570'),(578,'374320'),(581,'2358720'),(582,'236430'),(583,'2138330'),(584,'322330'),(585,'219740'),(586,'2239770'),(587,'712640'),(588,'393010'),(589,'282470')]

with open('steam_update.sql', 'w', encoding='utf-8') as f:
    ...
    for db_id, appid in appids:
        ...
        dlc_str = '[' + ','.join(str(d) for d in dlc_ids[:10]) + ']' if dlc_ids else '[]'
        
        # Categories (features)
        cats = data.get('categories', [])
        features = [c.get('description','') for c in cats if isinstance(c, dict) and c.get('description')]
        features_str = '[' + ','.join(f'"{f}"' for f in features) + ']' if features else '[]'
...
"""
        f.write(sql)
        print(f'  id={db_id}: dlc={len(dlc_ids)} features={len(features)}')
print('Done')
