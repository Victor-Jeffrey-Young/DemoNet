...
| [10](10-修复与上线计划.md) | 修复与上线计划 | 安全加固 + Schema 补全 + 架构清理（大部分已完成） |
| [11](11-后端架构分析.md) | 后端架构分析 | 23 项性能与安全风险评估（2026-07-04） |
| [12](12-后端架构改造计划.md) | 后端架构改造计划 | 4 阶段改造方案：安全防线 → 并发优化 → 性能瘦身 → 工程规范 |
| [13](13-测试用例.md) | 测试用例 | 前端 UI/UX + 后端稳定性/性能/安全验收用例 |
| [14](14-规范化重构计划.md) | 规范化重构计划 | 统一响应体 + DTO 校验 + 前端拦截器规范化 |
| [15](15-代码审计报告.md) | 代码审计报告 | 两轮审计合并（07-05 静态审查 + 07-08 五轴审查），25 项问题 + 修复状态追踪 |
| [16](16-CI-CD-GitHub-Actions.md) | CI/CD 工作流 | GitHub Actions 三阶段流水线（构建/测试/部署）+ GHCR 镜像 + SSH 部署 |
| [17](17-CI-CD-调试记录.md) | CI/CD 调试记录 | 首次推送后 3 轮调试（pnpm/Node 版本 + Spring Boot 4.x Flyway 自动配置变更），含诊断方法论 |
| [18](18-CI-CD修复计划.md) | CI/CD 修复计划 | 远程失败根因、工作流收敛方案、质量门禁、回滚与验收标准 |
...
详细计划见 [12-后端架构改造计划.md](12-后端架构改造计划.md)。
