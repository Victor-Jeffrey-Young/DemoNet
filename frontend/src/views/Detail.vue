...
const infoFields = computed(() => {
    const skip = [
        "demo_url", "benchmark_url", "videos", "dlc", "features", "languages",
        "min_requirements", "rec_requirements", "flavor_notes", "flavor",
        "total_cup_points", "aroma", "aftertaste", "acidity", "body",
        "balance", "uniformity", "clean_cup", "sweetness", "rule_images",
        "rule_text", "preview_url", "reader_url",
        "screenshots", "free", "is_free", "is_dlc", "demo_available"
    ];
    if (item.value?.type === 'offline') {
        skip.push("venue", "date", "time", "price", "capacity", "difficulty", "highlights");
    }
    const result = Object.entries(info.value)
        .filter(([k, v]) => !skip.includes(k) && v !== null && v !== "" && typeof v !== 'object' && typeof v !== 'boolean')
        .map(([k, v]) => {
            if (k === 'price') {
                let pv = v;
                if (String(v).toLowerCase() === 'free') pv = '免费 (Free)';
                return ["价格", pv];
            }
            if (k === 'pages') return ["页数", v];
            if (k === 'author') return ["作者", v];
            if (k === 'publisher') return ["出版商", v];
            if (k === 'developer') return ["开发商", v];
            if (k === 'platform') return ["平台", v];
            if (k === 'genre') return ["类型", v];
            if (k === 'release_date') return ["发售日", v];
            if (k === 'brand') return ["品牌", v];
            if (k === 'origin') return ["产地", v];
            if (k === 'roast_level') return ["烘焙度", v];
            if (k === 'processing_method') return ["处理法", v];
            if (k === 'artist') return ["艺术家", v];
            if (k === 'album') return ["专辑", v];
            if (k === 'isbn') return ["ISBN", v];
            if (k === 'players') return ["玩家数", v];
            if (k === 'play_time') return ["游玩时间", v];
        ...
        <main class="w-full max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-6 sm:py-10">
            ...
            <template v-else-if="item">
                <div class="flex items-center gap-3 mb-4 sm:mb-6">
                    ...
                    <!-- Video switcher -->
                    <div v-if="mediaHeroRef?.videoSources?.length > 1 && !mediaHeroRef?.readerUrl && !mediaHeroRef?.isMusic" class="flex items-center gap-1 overflow-x-auto scrollbar-hide">
                        <button v-for="src in mediaHeroRef.videoSources" :key="src.key" @click="mediaHeroRef.activeVideoUrl = src.url"
                            :class="mediaHeroRef.activeVideoUrl === src.url
                ...
                <div class="bg-gray-900 rounded-xl sm:rounded-2xl border border-gray-800 overflow-visible">
                    ...
                    <div class="p-4 sm:p-8">
                        <div class="flex items-center gap-3 mb-4">
                            <span :class="typeBadge" class="text-xs px-2 py-0.5 rounded">{{ typeMeta.label }}</span>
                            <span class="text-xs text-gray-600">{{ item.source }}</span>
                        </div>

                        <div class="flex items-start justify-between gap-2 sm:gap-4">
                            <h1 class="text-2xl sm:text-3xl font-bold mb-4 min-w-0 break-words">{{ item.title }}</h1>
                            <div class="relative shrink-0" @mouseenter="showDropdown" @mouseleave="hideDropdown">
                                <button :disabled="savingStatus"
                                    :class="myStatus ? 'bg-blue-600' : 'bg-gray-800 hover:bg-gray-700'"
                                    class="text-sm px-4 py-2 rounded-lg transition flex items-center gap-2">
                                    {{ currentStatusLabel }} ▾
                                </button>
                                <div v-show="dropdownOpen" class="absolute right-0 top-full mt-0.5 bg-gray-800 border border-gray-700 rounded-lg py-1 w-32 z-10"
                                    @mouseenter="showDropdown" @mouseleave="hideDropdown">
                                    <button v-for="opt in statusOptions" :key="opt.key"
                                        @click="setStatus(opt.key); dropdownOpen = false;"
                        ...
                        <component :is="typeComponentMap[item.type] || GenericDetail" :item="item" :info="info" />

                        <div v-if="infoFields.length > 0" class="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4 bg-gray-800/50 rounded-lg mt-6">
                ...
                <section v-if="related.length > 0" class="mt-16 border-t border-gray-800/60 pt-16">
            ...
            <div v-else class="text-center text-gray-500 py-20">
                ...
                <p>内容未找到</p>
                <button @click="goBack" class="mt-4 text-sm text-blue-400 hover:text-blue-300 transition">
                    ← 返回列表
                </button>
            </div>
        </main>
