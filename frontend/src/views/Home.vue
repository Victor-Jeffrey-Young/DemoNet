...
const categories = [
  ...
  { key: 'boardgame', label: '桌游', emoji: '🎲', desc: '规则教学 & 实况回放' },
    ...
    <main class="max-w-7xl mx-auto px-6">
      <section class="py-12">
        <div class="flex flex-col items-center text-center gap-4 mb-10">
          ...
          <p class="text-gray-400 text-lg max-w-lg">聚合游戏Demo、电影预告、桌游教学等一切能让你"先试试"的内容</p>
          <div class="w-full max-w-md mt-4"><SearchBar /></div>
        </div>

        <div v-if="mainBanner" class="mb-12">
          <div
            class="relative h-72 rounded-2xl overflow-hidden bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center cursor-pointer"
            @click="router.push(`/item/${mainBanner.slug}`)">
            <div class="absolute inset-0 bg-black/30" />
            <div class="relative z-10 text-center p-8">
              <span class="text-xs px-2 py-0.5 rounded bg-white/10 text-white/80 mb-3 inline-block">{{ mainBanner.type }}</span>
              <h2 class="text-3xl font-bold mb-2">{{ mainBanner.title }}</h2>
              <p class="text-gray-300 max-w-md mx-auto line-clamp-2">{{ mainBanner.description }}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section v-if="recommended.length > 0" class="mb-16">
        <div class="flex items-center justify-between mb-6">
          ...
          <router-link to="/list" class="text-blue-400 text-sm hover:underline">全部 →</router-link>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <AppCard v-for="item in recommended" :key="'rec-'+item.id" :item="item" />
        </div>
      </section>
      ...
      <section class="mb-16">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
          <div v-for="cat in categories" :key="cat.key"
            ...
            <div class="text-gray-500 text-xs mt-1 hidden sm:block">{{ cat.desc }}</div>
          </div>
        </div>
      </section>
      ...
      <section v-if="hotItems.length > 0" class="mb-16">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <AppCard v-for="item in hotItems" :key="item.id" :item="item" />
        </div>
      </section>
      ...
      <section v-if="hotByType.game?.length" class="mb-16">
        <div class="flex items-center justify-between mb-6">
          ...
          <router-link to="/list/game" class="text-blue-400 text-sm hover:underline">更多 →</router-link>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <AppCard v-for="item in hotByType.game" :key="item.id" :item="item" />
        </div>
      </section>
      ...
      <section v-if="hotByType.movie?.length" class="mb-16">
        <div class="flex items-center justify-between mb-6">
          ...
          <router-link to="/list/movie" class="text-blue-400 text-sm hover:underline">更多 →</router-link>
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <AppCard v-for="item in hotByType.movie" :key="item.id" :item="item" />
        </div>
      </section>
    </main>
  </div>
