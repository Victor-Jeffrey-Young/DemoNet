...
function next() { 
  ...
  active.value = (active.value + 1) % movies.value.length 
  resetTimer()
}

function prev() {
  if (!movies.value.length) return
  active.value = (active.value - 1 + movies.value.length) % movies.value.length
  resetTimer()
}
...
<template>
  <div class="relative w-full h-[calc(100vh-4rem)] overflow-hidden bg-black" v-if="movies.length" 
    ...
    <!-- Removed mode="out-in" to allow true crossfading without black flash -->
    <Transition name="hero-crossfade">
      <div :key="active" class="absolute inset-0 z-0"
        ...
        <div class="absolute inset-0 bg-gradient-to-r from-gray-950 via-gray-950/70 sm:via-gray-950/50 to-transparent" />
    ...
    <div class="relative z-10 h-full flex flex-col justify-center px-6 sm:px-12 max-w-4xl">
      <span class="text-red-400 text-xs sm:text-sm tracking-widest uppercase mb-4">{{ meta.label }}</span>
      <h1 class="text-4xl sm:text-7xl font-black tracking-tighter mb-3 drop-shadow-lg">{{ movies[active].title }}</h1>
      <p class="text-gray-300 text-base sm:text-lg max-w-xl mb-8 leading-relaxed line-clamp-3 sm:line-clamp-2">{{ movies[active].description }}</p>
      <div class="flex flex-col sm:flex-row gap-3 sm:gap-4">
        <button @click="goDetail(movies[active].slug)"
          class="px-6 py-3.5 sm:py-3 bg-red-600 hover:bg-red-500 rounded-xl text-sm font-semibold transition-colors shadow-lg shadow-red-900/30 text-center">
          查看详情
        </button>
        <button @click="watchTrailer(movies[active])"
          class="px-6 py-3.5 sm:py-3 border border-white/20 hover:border-white/40 rounded-xl text-sm transition-all text-center">
          ▶ 观看预告
        </button>
      </div>
    </div>
    ...
    <div class="absolute bottom-6 left-6 sm:bottom-8 sm:right-8 sm:left-auto z-20 flex items-center gap-3">
      <div class="flex gap-2">
        <button v-for="(m,i) in movies" :key="i" @click="active = i; resetTimer()"
