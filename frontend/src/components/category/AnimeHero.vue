  ...
  <div class="relative w-full h-[calc(100vh-4rem)] overflow-hidden bg-black" v-if="items.length">
    <Transition name="hero-crossfade" mode="out-in">
      <div :key="active" class="absolute inset-0 z-0"
        ...
        <div class="absolute inset-0 bg-gradient-to-r from-violet-950 via-fuchsia-950/70 to-transparent" />
    ...
    <div class="relative z-10 h-full flex flex-col justify-center px-12 max-w-4xl">
      <span class="text-violet-400 text-xs tracking-widest uppercase mb-4">{{ meta.label }}</span>
      <h1 class="text-5xl sm:text-7xl font-black tracking-tighter mb-3 drop-shadow-lg text-white">{{ items[active].title }}</h1>
      <p class="text-gray-300 text-lg max-w-xl mb-8 leading-relaxed line-clamp-2">{{ items[active].description }}</p>
      <div class="flex gap-4">
        <button @click="goDetail(items[active].slug)"
          class="px-6 py-3 bg-violet-600 hover:bg-violet-500 rounded-xl text-sm font-semibold transition-colors shadow-lg shadow-violet-900/30">
          查看详情
        </button>
      </div>
    </div>
    ...
    <div class="absolute bottom-8 right-8 z-20 flex items-center gap-3">
      <div class="flex gap-2">
        <button v-for="(m,i) in items" :key="i" @click="active = i"
...
.hero-crossfade-leave-active { transition: opacity 0.8s ease; }
