...
const meta = {
  game:      { label:'游戏', color:'bg-green-900/50 text-green-400', emoji:'🎮' },
  movie:     { label:'电影', color:'bg-red-900/50 text-red-400', emoji:'🎬' },
  anime:     { label:'动漫', color:'bg-purple-900/50 text-purple-400', emoji:'🎭' },
  boardgame: { label:'桌游', color:'bg-yellow-900/50 text-yellow-400', emoji:'🎲' },
  model:     { label:'模型', color:'bg-blue-900/50 text-blue-400', emoji:'🧩' },
...
function goDetail() {
  if (props.item?.slug) router.push({ name: 'Detail', params: { slug: props.item.slug } })
}
</script>
...
<template>
  <div @click.stop="goDetail"
    ...
    <div class="aspect-video bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center text-4xl relative overflow-hidden">
      ...
      <span class="relative z-10">{{ (meta[item.type]||{}).emoji || '📦' }}</span>
    </div>
    <div class="p-4">
      ...
      <h3 class="mt-2 font-semibold text-lg group-hover:text-blue-400 transition-colors">{{ item.title }}</h3>
